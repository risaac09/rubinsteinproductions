...
The public site at rubinsteinproductions.com: a React 19 + Vite + GSAP single-page app served by Netlify (the GitHub Pages deploy is a mirror, per CLAUDE.md and commit 9f87368, 2026-07-28) presenting Isaac Rubinstein's facilitation, film, and program evaluation practice. It carries the Say Why offering, the evaluation section, essays, films, a contact form, and the static /hr1-tracker page, and it absorbed the retired isaacrubinstein.com. Tier 2 in the estate, a public view; the spine is stack-data, Tier 1, the operational source of truth, a sibling clone at `../stack-data`. This file is the map of the repo's documentation plus the connective tissue that was missing, chiefly the content runbook and the external dependencies.

## What it is (technical)

React + Vite with react-router, GSAP scroll animations, Netlify production hosting with a GitHub Pages mirror, SPA fallback via `public/_redirects` on Netlify and via `public/404.html` decoded by the script in `index.html` on the Pages mirror. The stack and section map live in `README.md`; the framework-exception rule (React/Vite stays contained to this repo, no new framework surface, no TypeScript, no CSS framework, no dependencies beyond GSAP) lives in `CLAUDE.md`. The design-system contract, tokens, motion law, and the Reveal Contract live in `DESIGN.md` with source of truth in `src/styles/design-system.css`.
...
- **Services diagnostic iframe.** `src/pages/Services.jsx:371` embeds `https://alchemy.rubinsteinproductions.com/embed.html`, alchemy's consolidated embed, with a postMessage listener at lines 15-24 that checks that origin. The repointing away from the redundant standalone alchemy-diagnostic repo is done. Alchemy's `embed.html` posts `{type:'started'}`, `{type:'email',email}`, and `{type:'complete',scores,quadrant}` from its lead-capture funnel.
...
Netlify serves production; the deploy runs via the CLI recipe in the netlify-deploy memory, and `public/_redirects` is the Netlify SPA fallback (re-added in commit 9f87368, 2026-07-28). Push to `main` runs `.github/workflows/deploy.yml`: Node 20, `npm ci`, Vite build, publish `dist/` to GitHub Pages as a mirror, not the live site. `public/CNAME` pins the custom domain. Local commands are `npm run dev`, `npm run build`, `npm run preview`. No repository secrets beyond the workflow's default GitHub token.
...
Consolidation closeout: `CONSOLIDATION-PROMPT.md` lists the remaining Mac-only credential steps (keep isaacrubinstein.com registered with per-URL redirect stubs, Google Search Console change of address, forward isaac@isaacrubinstein.com, re-export cv.pdf). Its "prove the apex serves GitHub Pages / disconnect the Netlify site" step is superseded: as of 2026-07-28 Netlify is confirmed as the production host and stays connected.
...
- Resolved: the Services iframe now points at alchemy's consolidated `embed.html` (`https://alchemy.rubinsteinproductions.com/embed.html`), so the alchemy-diagnostic repo can archive without breaking the page.
...
Gap: whether the outreach cadence in `outreach-system.md` (March 2026, in rp-intranet) still runs as written after the consolidation replaced the Mirror offering; the answer would come from Isaac or the stack-data activity log.

## Workflows

Automated:

- `.github/workflows/deploy.yml`: on push to `main` and on `workflow_dispatch`. Node 20, `npm ci`, Vite build, publish `dist/` to GitHub Pages with the custom domain from `public/CNAME`. The Pages deploy is a mirror; Netlify serves production and ships via the CLI recipe, not this workflow. No named secrets beyond the default workflow token.
...
- **Consolidation closeout:** Isaac runs the remaining steps from `CONSOLIDATION-PROMPT.md` on the M2 Pro (redirect stubs, GSC change of address, mailbox forward, cv.pdf re-export). The dig/curl proof and Netlify-disconnect step is superseded as of 2026-07-28: Netlify is the production host and stays connected. Good looks like each remaining step dated done in that file.
- **Outreach cadence** per `outreach-system.md` (in rp-intranet): daily LinkedIn engagement, at most 3 Mirror emails per week, invitations as earned, twice-weekly LinkedIn posts from the brand-card series. Content status above notes this doc predates the current offerings.
...
1. Resolved: `src/pages/Services.jsx:371` now embeds `https://alchemy.rubinsteinproductions.com/embed.html`, alchemy's consolidated embed, and the postMessage listener checks that origin. Archiving the standalone alchemy-diagnostic repo no longer breaks the Services page.
...
3. `ROADMAP-SEO.md` reads as instructions but describes a finished build with errors: a `/work` route and `src/pages/Work.jsx` that do not exist, `og-image.jpg` versus the shipped `og-card.png`, a sample sitemap with the typo domain `rubensteinproductions.com` and wrong xmlns, `risaac09@gmail.com` as contact fallback versus the site's isaac@rubinsteinproductions.com, title format `X — Site` versus the shipped `X · Site`, and jobTitle `Voice Liberation Specialist` versus the shipped `Facilitator, Filmmaker & Program Evaluator`.
4. `CONSOLIDATION-PROMPT.md` body still describes a Netlify README claim and a `public/_redirects` file; the README claim was fixed, but `_redirects` was deliberately re-added on 2026-07-28 (commit 9f87368) as the Netlify SPA fallback, since Netlify serves production. Its own June 2026 status note supersedes the body. Its redirect map targets `/work/central-providence`, a route never built.
5. `outreach-system.md` (March 2026, in rp-intranet) frames the offering around the retired Mirror 90-minute session and is written with em-dashes throughout, at odds with the repo's voice discipline for an internal doc.
